import pandas as pd
import numpy as np
from sklearn.model_selection import train_test_split
from sklearn.linear_model import LogisticRegression
from sklearn.metrics import accuracy_score

# Data Collection and Data Processing:
# Loading the dataset to pandas dataframe(df):
sonar_data = pd.read_csv("Copy of sonar data.csv", header=None)
print(sonar_data.head())
# print(sonar_data.shape)
'''
(208, 61)
208 rows
61 columns
'''
# print(sonar_data[60].value_counts())
'''
60
M    111    (mines)
R     97    (rocks)
Name: count, dtype: int64
'''
# print(sonar_data.groupby(60).mean())
'''
         0         1         2   ...        57        58        59
60                                ...                              
M   0.034989  0.045544  0.050720  ...  0.009060  0.008695  0.006930
R   0.022498  0.030303  0.035951  ...  0.006677  0.007078  0.006024

'''
# Separating data and label:
X = sonar_data.drop(columns=60, axis=1)
y = sonar_data[60]

# splitting training and testing data:

X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.1, stratify=y, random_state=1)
'''
test_size = 0.1 means we need 10% of 
data to be test data ,random_state is 
to split the data into particular order
 to reproduce the code a sit is.
'''
# print(X.shape, X_train.shape, X_test.shape)
'''
(208, 60) (187, 60) (21, 60)
'''
# Logistic Regression model:

our_model = LogisticRegression()

# Training the model with training data:
our_model.fit(X_train, y_train)

# Model Evaluation:
# accuracy on the training data:
X_train_predict = our_model.predict(X_train)
training_accuracy = accuracy_score(X_train_predict, y_train)

print(f"Accuracy of training data: {training_accuracy}")
'''
Accuracy of training data: 0.8342245989304813
'''

# accuracy on the test data:
X_test_predict = our_model.predict(X_test)
test_accuracy = accuracy_score(X_test_predict, y_test)

print(f"Accuracy of test data: {test_accuracy}")
'''
Accuracy of test data: 0.7619047619047619
'''

# Making a predictive system
input_data = (0.0200,0.0371,0.0428,0.0207,0.0954,0.0986,0.1539,0.1601,0.3109,0.2111,0.1609,0.1582,0.2238,0.0645,0.0660,0.2273,0.3100,0.2999,0.5078,0.4797,0.5783,0.5071,0.4328,0.5550,0.6711,0.6415,0.7104,0.8080,0.6791,0.3857,0.1307,0.2604,0.5121,0.7547,0.8537,0.8507,0.6692,0.6097,0.4943,0.2744,0.0510,0.2834,0.2825,0.4256,0.2641,0.1386,0.1051,0.1343,0.0383,0.0324,0.0232,0.0027,0.0065,0.0159,0.0072,0.0167,0.0180,0.0084,0.0090,0.0032)

# Changing input_data to numpy array:
input_data_to_numpy_array = np.asarray(input_data)

# Reshaping numpy array:
input_data_reshaped = input_data_to_numpy_array.reshape(1, -1)

prediction = our_model.predict(input_data_reshaped)

if prediction[0] == 'R':
    print("The Object is a Rock🪨")
else:
    print("The Object is a Mine💣")
